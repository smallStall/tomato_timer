...
import TomatoSlider from "../components/templates/TomatoSlider";
...
function makeNotifyMessage(count: number, memo: string, oneSet : number){
  ...
  if(oneSet === 1){
    message = pomodoroCount === count ? 'お休みに移ります。' : 'お休みが終わりました。';
  }else if(pomodoroCount !== oneSet){
      message = count % 2 !== 0
      ? (oneSet.toString() + '回中' + pomodoroCount.toString() + '回ポモドーロが終わりました。お休みに移ります。')
      : 'お休みが終わりました。作業に取り掛かりましょう。';
  }else{
    message = oneSet.toString() + '回ポモドーロを達成しました。お疲れ様でした。'
  }
...
const Home: NextPage = () => {
  ...
  const [interval, setInterval] = useState(990);
